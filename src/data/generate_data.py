...
def sample_tsp(
    batch_size: int, n: int, device: torch.device | None = None
) -> torch.Tensor:
...
def sample_cvrp(
    ...
    loc = torch.rand(batch_size, n, 2, device=device)
    raw_demand = torch.randint(
        1, 10, (batch_size, n), device=device, dtype=torch.float32
    )
...
def sample_op(
    ...
    if distribution == "const":
        ...
        prize = (
            torch.randint(1, 101, (batch_size, n + 1), device=device).float() / 100.0
        )
...
def sample_pctsp(
    ...
    raw_prize = torch.rand(batch_size, n + 1, device=device)
    raw_prize[:, 0] = 0
    prize = raw_prize * (4.0 / n)

    penalty = torch.rand(batch_size, n + 1, device=device) * (3.0 * k_n / n)
    penalty[:, 0] = 0
    ...
    if stochastic:
        ...
        out["real_prize"] = torch.rand(batch_size, n + 1, device=device) * 2.0 * prize
